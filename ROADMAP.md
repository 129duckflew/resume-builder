...
| ✅ 11. 主题变量化与样式配置 (G19-A) | 已完成 | `c66c9d5` |

> 已完成目标详情见 `progress.md`。测试基线 228/228 ✅，E2E core-flow 3 specs ✅。
...
将主题模型从纯 CSS 升级为变量驱动的声明式架构，支持运行时自定义与多布局。

| # | 子目标 | 状态 | 提交 |
|---|---|---|---|
| A | 主题变量化 — Theme/ResumeStyle 变量字段、ExportService :root 注入、ThemeCustomizer 组件、7 主题 var() 化 | ✅ 已完成 | `c66c9d5` |
| B | 布局模板与渲染重构 — 修复 sidebar 主题失效 + 支持多布局（双栏/侧栏） | ⏳ 待办 | — |
| C | 主题分类预览与自定义主题 — 前端分类展示 + 用户自定义主题持久化 | ⏳ 待办 | — |

**依赖**：G19-B 会冲击渲染契约（`@page` / `.resume-page` 假设），需先做 P2 评估。G19-C 依赖 G19-B 布局稳定。
